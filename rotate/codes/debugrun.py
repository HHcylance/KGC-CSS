...
def parse_args(args=None):
    ...
    parser.add_argument('--get_metric', action='store_true')
    parser.add_argument('--safe_ranking', action='store_true')
    ...
    # parser.add_argument('-adv', '--negative_adversarial_sampling', action='store_false')
    ...
    parser.add_argument('-init', '--init_checkpoint', default='/home/hubinhao/store/StAR_KGC-master/rotate/models/RotatE_wn18rr_0', type=str)
    ...
    parser.add_argument('--typecons', action='store_true')
    parser.add_argument('--get_scores', action='store_false')  #
    parser.add_argument('--get_cases', action='store_true')
    parser.add_argument('--get_hits_info', action='store_true')
    parser.add_argument('--get_model_dataset', action='store_false')  #
    parser.add_argument('--star_info_path', type=str,
                        default='/home/hubinhao/store/StAR_KGC-master/StAR/result/WN18RR_roberta-large')
...
def main(args):
    ...
    if args.do_train:
        ...
        train_dataloader_head = DataLoader(
            TrainDataset(train_triples, nentity, nrelation, args.negative_sample_size, 'head-batch', entity2id,
        ...
        train_dataloader_tail = DataLoader(
            TrainDataset(train_triples, nentity, nrelation, args.negative_sample_size, 'tail-batch', entity2id,
                         relation2id, args.data_path, args.typecons),
            batch_size=args.batch_size,
            shuffle=True,
            num_workers=max(1, args.cpu_num // 2),
    ...
    if args.get_metric:
        logging.info('Evaluating on Test Dataset and Show the metric in two sides...')
        head_metrics, tail_metrics = kge_model.get_metric(kge_model, test_triples, all_true_triples, args)
        logging.info("--------------- Head ------------\n")
    ...
    if args.rel_do_test:
        ...
        for _rel in relation2id:
            test_ex_dict_departby_rel[_rel] = [_ex for _ex in test_triples if _ex[1] == relation2id[_rel]]

        for _rel in test_ex_dict_departby_rel.keys():
            _rel_test_triples = test_ex_dict_departby_rel[_rel]
            _rel_data = [train_rel_dict[relation2id[_rel]], len(_rel_test_triples)]
            if len(_rel_test_triples) != 0:
                metrics = kge_model.test_step(kge_model, _rel_test_triples, all_true_triples, args)
                _rel_data.extend([round(metrics['HITS@1'], 3), round(metrics['HITS@3'], 3),
                                  round(metrics['HITS@10'], 3), round(metrics['MR'], 1),
                                  round(metrics['MRR'], 3)])
            else:
                _rel_data.extend([0, 0, 0, 0, 0])
            rel_dict[_rel] = _rel_data

        sorted_rel = sorted(rel_dict.items(), key=lambda x: x[1][0], reverse=True)

        save_dir = args.init_checkpoint
        with open(join(save_dir, "rel_unbalanced.txt"), "w", encoding="utf-8") as fp:
