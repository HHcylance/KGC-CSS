...
from kbc import hyperbolic as hy
...
class BertForPairScoring(BertPreTrainedModel):
    ...
    @staticmethod
    def hyperbolic_distance(rep1, rep2):
        distance = hy.hyp_distance(rep1, rep2, 1)
        distance = torch.norm(distance, p=2, dim=-1)
        return distance
...
class RobertaForPairScoring(BertPreTrainedModel):
    ...
    def hyperbolic_distance(rep1, rep2):
        distance = hy.hyp_distance(rep1, rep2, 1)
        distance = torch.norm(distance*100, p=2, dim=-1)
        return distance
    ...
    def __init__(self, config):
        ...
        if self.distance_metric == "cosine":
            ...
            self.distance_metric_fn = self.euclidean_distance
        elif self.distance_metric == "hyperbolic":
            self.distance_metric_fn = self.hyperbolic_distance
        elif self.distance_metric == "bilinear":
            self.dropout = nn.Dropout(config.hidden_dropout_prob)
            self.bilinear = nn.Bilinear(config.hidden_size, config.hidden_size, 1, False)
            self.distance_metric_fn = lambda rep1, rep2: self.bilinear(
    ...
    def forward(self, src_input_ids, tgt_input_ids,
        ...
        if label_dict is not None:
            ...
            # The final loss to train the StAR is a weighted sum of this loss and the loss derived from BERT/RoBERTa classifier.
